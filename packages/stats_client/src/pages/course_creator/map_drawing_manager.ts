...
export class MapDrawingManager {
      ...
      if (type === 'create') {
        if (
          ...
          (f.properties.type === 'trajectory' ||
            this.getCurrentMode() === 'draw_line_string')
        ) {
  ...
  private async saveDrawState(): Promise<void> {
    ...
    try {
      console.log(currentState, currentMetadata);
  ...
  public async undo(): Promise<void> {
    ...
    try {
      ...
      const currentState = this.draw.getAll();
      const currentMetadata = this.callbacks.getCurrentHoleMetadata?.() || {};
      const previousStateData = await this.undoRedoStorage.popUndo(
        currentState,
        currentMetadata,
      );
      ...
      if (previousStateData) {
        ...
        if (this.callbacks.onHoleStateRestore) {
          this.callbacks.onHoleStateRestore(
            previousStateData.state,
            previousStateData.holeMetadata,
          );
        }
  ...
  public async redo(): Promise<void> {
    ...
    try {
      ...
      const currentState = this.draw.getAll();
      const currentMetadata = this.callbacks.getCurrentHoleMetadata?.() || {};
      const nextStateData = await this.undoRedoStorage.popRedo(
        currentState,
        currentMetadata,
      );
  ...
  public async getStackInfo(): Promise<{
    undoSize: number;
    redoSize: number;
  } | null> {
