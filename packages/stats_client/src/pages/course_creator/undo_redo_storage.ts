...
export class UndoRedoStorage {
   ...
   */
  private async getLatestStackItem(
    stackType: 'undo' | 'redo',
  ): Promise<UndoRedoStackItem | undefined> {
   ...
   */
  private async getMaxSequenceIndex(
    stackType: 'undo' | 'redo',
  ): Promise<number> {
  ...
  private async trimStack(stackType: 'undo' | 'redo'): Promise<void> {
    ...
    if (items.length > MAX_STACK_SIZE) {
      ...
      const idsToRemove = itemsToRemove.map(
        (item: UndoRedoStackItem) => item.id!,
      );
 ...
 */
export function createUndoRedoStorage(
  courseId: number,
  holeNumber: number,
): UndoRedoStorage {
