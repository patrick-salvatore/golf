...
export function createForm(options = {} as any) {
  ...
  const reset = () => {
    batch(() => {
      form._submitted.set(false);
      form._submitting.set(false);
      form._touched.set(false);
      ...
      names.forEach((name) => {
        ...
        if (field) {
          field._value.set(field.initialValue.get());
          field.touched.set(false);
          field.dirty.set(false);
          field._error.set(undefined);
        }
      });
    });
  };
  ...
  return { form, register, handleSubmit, reset };
}
