        ...
        <Select
          ...
          itemComponent={props => (
            <SelectItem item={props.item}>
              {tournamentsQuery.data.find(t => t.id === props.item.rawValue)?.name}
            </SelectItem>
          )}
        >
          <SelectTrigger>
            <SelectValue>{state => tournamentsQuery.data.find(t => t.id === state.selectedOption())?.name}</SelectValue>
          </SelectTrigger>
          <SelectContent />
        </Select>
      </div>
      ...
      <Show when={generatedLink()}>
        <div class="p-4 bg-gray-50 border rounded-lg space-y-2">
          ...
          <p class="text-xs text-gray-500">
            Share this link with players. They can join the tournament and select their team.
          </p>
        </div>
      </Show>
    </div>
